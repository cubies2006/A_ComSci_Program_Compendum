...
public class CalcOrig extends Applet
{
   public void init()
   {  
	...
	addButton("-", Color.lightGray, binary);
	addButton("(-)", Color.lightGray, unary);
	addButton("\u00d7", Color.lightGray, binary);	// multiply button
	addButton("\u00f7", Color.lightGray, binary);	// divide button

	addButton("(", Color.gray, unary);
	addButton(")", Color.gray, binary);
	addButton("Mem", Color.gray, unary);
	addButton("MR", Color.gray, unary);
	addButton("M+", Color.gray, unary);

	addButton("=", Color.green, binary);
 	addButton("\u03c0", Color.cyan, unary);  	// pi button
	addButton("x\u00b2", Color.cyan, unary);	// square button
	addButton("\u221Ax", Color.cyan, unary);	// square root button
	addButton("Clr", Color.red, unary);

	addButton("sin", Color.cyan, unary);
	addButton("cos", Color.cyan, unary);
	addButton("tan", Color.cyan, unary);
	addButton("exp", Color.cyan, unary);
	addButton("10^x", Color.cyan, unary);

	

	addButton("sin\u2212\u00b9", Color.cyan, unary);	// arcsine button
	addButton("cos\u2212\u00b9", Color.cyan, unary);	// arccos button
	addButton("tan\u2212\u00b9", Color.cyan, unary);	// arctan button
	addButton("ln", Color.cyan, unary);
	addButton("log", Color.cyan, unary);

	addButton("1/x", Color.cyan, unary);
	addButton("x^y", Color.cyan, binary);
	addButton("x!", Color.cyan, unary);
	addButton("nPr", Color.cyan, binary);
	addButton("nCr", Color.cyan, binary);
   ...
   */
   ...
   {	
	...
	else if (lastCommand.equals("nPr")) result = factorial(stored)/factorial(stored - x);
	else if (lastCommand.equals("nCr")) result = factorial(stored)/factorial(stored-x)/factorial(x); 
   ...
   public void operate(double x)
   {
	...
	else if (command.equals("exp")) result = Math.exp(x);
	else if (command.equals("ln")) result = Math.log(x);
        else if (command.equals("(-)")) result = x * -1;
	else if (command.equals("sin")) result = Math.sin(factor*x);
	else if (command.equals("sin\u2212\u00b9")) result = Math.asin(x)/factor;

	else if (command.equals("cos" )) result = Math.cos(factor*x);
	else if (command.equals("cos\u2212\u00b9")) result = Math.acos(x)/factor;

	else if (command.equals("tan")) result = Math.tan(factor*x);
	else if (command.equals("tan\u2212\u00b9")) result = Math.atan(x)/factor;
	else if (command.equals("1/x")) result = 1/x;	
	else if (command.equals("10^x")) result = Math.pow(10, x);
	else if (command.equals("x!")) result = factorial(x);
	else if (command.equals("log")) result = Math.log(x)/Math.log(10);
	else if (command.equals("\u03c0")){ result = Math.PI; start = false; }
	else if (command.equals("Mem")) { memory = x; mem.setText("" + memory); result = x;}
	else if (command.equals("M+")) { memory += x; mem.setText("" + memory); result = x; start = true;}
	else if (command.equals("MR")) { result = memory; }

	if (command.equals("(")) 
		{ 	
			level++;
			stack[level] = result; 
			stackedCommand[level] = lastCommand;
			lastCommand = "="; 
			stored = 0;
			result = 0;
			inputting = true;
